...
export function DashboardSidebar({ userPlan = 'free', isAdmin = false }: DashboardSidebarProps) {
  ...
  return (
    <aside className="w-16 flex flex-col h-full" style={{ background: '#1C1C2E' }}>
      {/* Logo */}
      ...
      <nav className="flex flex-col gap-0.5 px-2 flex-1 pt-2 overflow-y-auto">
        {NAV_ITEMS.map(item => {
          ...
          return (
            <Link
              ...
              title={locked ? `${item.label} — Plan Premium requis` : item.label}
            >
              ...
              {locked && (
                <span className="text-[6px] font-bold px-1 py-px rounded-full" style={{ background: '#EBF2FF', color: '#3183F7', lineHeight: 1.4 }}>
                  Pro
                </span>
              )}
            </Link>
      ...
      <div className="px-2 pb-3 flex flex-col gap-0.5">
        ...
        {isAdmin && (
          ...
          >
            ...
            <span className="text-[8px] font-semibold" style={{ color: '#F56751' }}>Admin</span>
          </Link>
        )}
        ...
        {[
          { href: '/', label: 'Site', icon: <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M2 7l6-5 6 5v7a1 1 0 01-1 1H3a1 1 0 01-1-1V7z" stroke="rgba(255,255,255,.4)" strokeWidth="1.3" strokeLinejoin="round"/></svg> },
          { href: '/documentation', label: 'Docs', icon: <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="2" y="2" width="12" height="12" rx="2" stroke="rgba(255,255,255,.4)" strokeWidth="1.3"/><path d="M5 6h6M5 9h4" stroke="rgba(255,255,255,.4)" strokeWidth="1.3" strokeLinecap="round"/></svg> },
          { href: '/articles', label: 'Articles', icon: <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M3 4h10M3 7h10M3 10h6" stroke="rgba(255,255,255,.4)" strokeWidth="1.3" strokeLinecap="round"/></svg> },
        ].map(({ href, label, icon }) => (
