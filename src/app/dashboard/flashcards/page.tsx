...
const BUILT_IN_CARDS: Record<string, Array<{ front: string; back: string }>> = {
  ...
  'futures-forwards': [
    ...
    { front: 'Qu\'est-ce que le Contango ?',                                 back: 'Le prix des futures lointains est supérieur au spot. Situation normale pour les actifs stockables (pétrole, métaux). Reflète le coût de portage.' },
    ...
    { front: 'Qu\'est-ce que le roll yield pour un ETF futures ?',           back: 'Gain ou perte lors du renouvellement mensuel des contrats. En contango : perte (on vend bas, on rachète haut). En backwardation : gain.' },
    { front: 'Comment couvrir un portefeuille actions avec des futures ?',   back: 'N contrats = (β_portefeuille × Valeur portefeuille) / Valeur notionnelle future. On vend les futures pour réduire le beta.' },
    { front: 'Qu\'est-ce qu\'un FRA (Forward Rate Agreement) ?',             back: 'Contrat forward sur taux d\'intérêt. Fixe aujourd\'hui le taux applicable à un emprunt futur. Réglé en cash à l\'échéance sur la différence de taux.' },
    { front: 'Qu\'est-ce que l\'arbitrage cash-and-carry ?',                 back: 'Si le future est surévalué : emprunter, acheter le spot, vendre le future → profit sans risque à l\'échéance. Les arbitragistes maintiennent F = S₀ × e^{rT}.' },
  ],
}
