...
const FALLBACK: Record<string, FallbackQuestion[]> = {
  finance: [
    {
...
export async function GET(req: NextRequest) {
  ...
  const pool: Array<{ text: string; explanation: string; domain: string; level: number; answers: { text: string; isCorrect: boolean }[] }> = []
    ...
    if (sanityData?.length) {
      for (const quiz of sanityData) {
        ...
        for (const q of quiz.questions ?? []) {
          pool.push({ text: q.text, explanation: q.explanation ?? '', domain: quiz.domain, level, answers: q.answers })
        }
      }
    }
